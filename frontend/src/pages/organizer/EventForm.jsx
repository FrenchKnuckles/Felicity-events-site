...
const AREAS = [
  ...
  "Finance & Trading", "Social Events", "Environment",
];
const FIELD_TYPES = [
...
const INIT_DATA = {
  ...
  purchaseLimitPerUser: 5, minTeamSize: "", maxTeamSize: "",
};
...
const EventForm = ({ mode = "create" }) => {
  ...
  useEffect(() => { if (isEdit && id) fetchEvent(); }, [id]);
  ...
  const fetchEvent = async () => {
    try {
      ...
      setEd({
        ...
        variants: e.variants || [], purchaseLimitPerUser: e.purchaseLimitPerUser || 5,
  ...
  const updDate = (key, dp) => setEd((p) => {
    ...
    return { ...p, [key]: dp ? `${dp}T${time || "00:00"}` : `T${time}` };
  });
  const updTime = (key, tp) => setEd((p) => {
    const { date } = splitDT(p[key]);
    if (!date && !tp) return { ...p, [key]: "" };
    if (!date) return { ...p, [key]: tp ? `T${tp}` : "" };
    return { ...p, [key]: `${date}T${tp || "00:00"}` };
  });
  const onChange = (e) => {
    const { name, value, type } = e.target;
    setEd((p) => ({ ...p, [name]: type === "number" ? (value === "" ? "" : parseFloat(value)) : value }));
  };
  ...
  const updVariant = (i, f, v) => setEd((p) => ({ ...p, variants: p.variants.map((x, j) => j === i ? { ...x, [f]: (f === "price" || f === "stock") ? parseFloat(v) || 0 : v } : x) }));
  const rmVariant = (i) => setEd((p) => ({ ...p, variants: p.variants.filter((_, j) => j !== i) }));

  const validate = () => {
    if ((!isEdit && step === 1) || (isEdit && activeTab === "details")) {
      if (!ed.name || !ed.description) { toast.warning("Please fill in all required fields"); return false; }
    }
    if ((!isEdit && step === 2) || (isEdit && activeTab === "details")) {
      if (!ed.startDate || !ed.registrationDeadline) { toast.warning("Please fill in date fields"); return false; }
      const s = new Date(ed.startDate), r = new Date(ed.registrationDeadline);
      if (isNaN(s.getTime()) || isNaN(r.getTime())) { toast.warning("Invalid dates"); return false; }
      if (r > s) { toast.warning("Registration deadline must be before start"); return false; }
    }
    if (ed.eventType === "merchandise" && ed.variants.length === 0 && ((!isEdit && step === 3) || isEdit)) {
      toast.warning("Add at least one variant"); return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (publish = false) => {
    if (!validate()) return;
    const setLoad = isEdit ? setSaving : setSaving;
    setLoad(true);
    try {
      const payload = {
        ...ed, registrationFee: ed.registrationFee === "" ? 0 : Number(ed.registrationFee),
        registrationLimit: ed.registrationLimit === "" ? undefined : Number(ed.registrationLimit),
        purchaseLimitPerUser: ed.purchaseLimitPerUser === "" ? undefined : Number(ed.purchaseLimitPerUser),
      ...
      if (isEdit) {
        ...
        } else toast.success(publish ? "Event created as draft" : "Event saved as draft!");
      ...
      <Box>
        ...
        <Grid columns="2" gap="3" mt="2">
          {[["normal", "Normal Event", "Workshop, Competition", CalendarIcon, "blue"], ["merchandise", "Merchandise", "T-shirts, Goodies", CubeIcon, "purple"]].map(([val, title, sub, Icon, color]) => (
            <Box key={val} onClick={() => setEd((p) => ({ ...p, eventType: val }))}
              style={{ padding: 12, borderRadius: 8, border: `2px solid ${ed.eventType === val ? `var(--${color}-9)` : "var(--gray-5)"}`, backgroundColor: ed.eventType === val ? `var(--${color}-2)` : "transparent", cursor: "pointer", textAlign: "center" }}>
  ...
  const formBuilderFields = (
    <Flex direction="column" gap="4">
      {ed.eventType === "merchandise" ? (
        <>
          <Flex align="center" gap="2"><CubeIcon width="20" height="20" /><Heading size="4">Merchandise Variants</Heading></Flex>
          <Box>
          ...
          {ed.variants.map((v, i) => (
            <Card key={i} variant="surface">
              <Flex justify="between" align="center" mb="2">
                ...
                <Button type="button" variant="ghost" color="red" size="1" onClick={() => rmVariant(i)}><TrashIcon /></Button>
              </Flex>
              <Grid columns={{ initial: "2", md: "4" }} gap="3">
                {[["Size", "size", "S, M, L"], ["Color", "color", "Black"], ["Price (₹)", "price", "0"], ["Stock", "stock", "0"]].map(([l, f, ph]) => (
                  <Box key={f}>
                    <Text as="label" size="1" weight="medium">{l}</Text>
                    <TextField.Root type={f === "price" || f === "stock" ? "number" : "text"} value={v[f]} onChange={(e) => updVariant(i, f, e.target.value)} placeholder={ph} size="2" mt="1" />
                  </Box>
                ))}
              </Grid>
            </Card>
          ))}
          <Button type="button" variant="outline" onClick={addVariant}><PlusIcon /> Add Variant</Button>
        </>
      ) : (
        <>
          ...
          {ed.customForm.map((f, i) => (
            <Card key={i} variant="surface">
              <Flex justify="between" align="center" mb="2">
                ...
                <Flex gap="1">
                  <Button type="button" variant="ghost" size="1" onClick={() => moveField(i, -1)} disabled={i === 0}><ArrowUpIcon width="14" height="14" /></Button>
                  <Button type="button" variant="ghost" size="1" onClick={() => moveField(i, 1)} disabled={i === ed.customForm.length - 1}><ArrowDownIcon width="14" height="14" /></Button>
                  <Button type="button" variant="ghost" color="red" size="1" onClick={() => rmField(i)}><TrashIcon /></Button>
                </Flex>
              </Flex>
              ...
              {["dropdown", "select", "radio"].includes(f.fieldType) && (
                <Box mt="3">
                  <Text as="label" size="1" weight="medium">Options (comma-separated)</Text>
                  <TextField.Root value={f.options?.join(", ")} onChange={(e) => updField(i, "options", e.target.value.split(",").map((o) => o.trim()))} placeholder="Option 1, Option 2" size="2" mt="1" />
                </Box>
              )}
            </Card>
          ))}
    ...
    <Flex direction="column" gap="4">
      ...
      <Card variant="surface" style={{ backgroundColor: "var(--gray-2)" }}>
        ...
        <Grid columns="2" gap="2">
          {[["Name", ed.name], ["Type", ed.eventType], ["Eligibility", ed.eligibility], ["Fee", `₹${ed.registrationFee}`], ["Venue", ed.venue || "N/A"], ["Limit", ed.registrationLimit || "Unlimited"]].map(([l, v]) => (
            <Text size="2" key={l}><Text color="gray">{l}:</Text> {v}</Text>
          ))}
        </Grid>
      </Card>
      <Card variant="surface" style={{ backgroundColor: "var(--gray-2)" }}>
        ...
        <Grid columns="2" gap="2">
          ...
          <Text size="2"><Text color="gray">End:</Text> {ed.endDate ? new Date(ed.endDate).toLocaleString() : "Same day"}</Text>
          <Text size="2"><Text color="gray">Deadline:</Text> {new Date(ed.registrationDeadline).toLocaleString()}</Text>
        </Grid>
      </Card>
      {ed.eventType === "merchandise" && ed.variants.length > 0 && (
        <Card variant="surface" style={{ backgroundColor: "var(--gray-2)" }}>
          <Text weight="medium" mb="2">Variants ({ed.variants.length})</Text>
          {ed.variants.map((v, i) => <Text size="2" key={i}>Size: {v.size}, Color: {v.color}, ₹{v.price}, Stock: {v.stock}</Text>)}
        </Card>
      )}
      ...
      <Flex align="center" justify="center" mb="6">
        {[1, 2, 3, 4].map((s) => (
          <Flex key={s} align="center">
            <Flex align="center" justify="center" style={{ width: 36, height: 36, borderRadius: "50%", backgroundColor: s <= step ? "var(--blue-9)" : "var(--gray-4)", color: s <= step ? "white" : "var(--gray-9)", fontWeight: 500 }}>{s}</Flex>
